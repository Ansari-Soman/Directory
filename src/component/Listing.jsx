...
const BusinessListingForm = () => {
  ...
  useEffect(() => {
    ...
    // Set roads
    updates.roads = dataObj.cityData.roads || [];

    // Set road-dependent data
    if (formData.road && dataObj.cityData.roads) {
      const selectedRoad = dataObj.cityData.roads.find(
        (road) => road.u_road_name === formData.road
      );
      if (selectedRoad) {
        updates.subRoads = selectedRoad.subRoads || [];
        updates.areas = selectedRoad.areas || [];
      }
    }

    // Set area-dependent data
    if (formData.area && updates.areas) {
      const selectedArea = updates.areas.find(
        (area) => area.u_area_name === formData.area
      );
      if (selectedArea) {
        updates.subAreas = selectedArea.subAreas || [];
      }
    }

    // Set category-dependent data
    if (formData.category && dataObj.category) {
      const selectedCategory = dataObj.category.find(
        (cate) => cate.u_business_category === formData.category
      );
      if (selectedCategory) {
        updates.subCategories = selectedCategory.subCategories || [];
      }
    }

    // Set sub-category-dependent data
    if (formData.subCategory && dataObj.subCategories) {
      const selectedSubCategory = dataObj.subCategories.find(
        (cate) => cate.u_business_sub_category === formData.subCategory
      );
      if (selectedSubCategory) {
        updates.categoryTypes = selectedSubCategory.categoryTypes || [];
      }
    }
    // console.log("Update === ", updates, dataObj)
    setDataObj((prev) => ({ ...prev, ...updates }));
  }, [
    dataObj.cityData,
    formData.road,
    formData.area,
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 1:
        return <ListingBusiName cities={dataObj.cityList} />;
      ...
      case 11:
        return <SubmitBusiness formData={formData} />;
      default:
        return 0;
    }
  };
